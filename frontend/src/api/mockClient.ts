...
async function streamCallOutputs(
  eventId: string,
  onActivity?: (activity: QueryActivity) => void,
): Promise<unknown[]> {
  ...
  try {
    ...
    for (;;) {
      ...
      while (frameEnd) {
        ...
        if (parsed?.event === "complete") {
          // Genuine backend signal: the Space has produced its output payload.
          onActivity?.("responding");
        ...
        // heartbeat / generating frames — the GPU is actually working; keep reading
        if (parsed?.event === "generating" || parsed?.event === "heartbeat") {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new Error(`Gradio queue timed out after ${CALL_TIMEOUT_MS / 1000}s`, { cause: err });
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
async function submitQueryViaGradio(
  ...
  // Genuine backend signal: the job is in the Space's queue.
  onActivity?.("queued");

  const outputs = await streamCallOutputs(eventId, onActivity);
  return normalizeGradioOutputs(outputs);
}

async function submitQueryViaFastapi(
  request: QueryRequest,
  onActivity?: (activity: QueryActivity) => void,
): Promise<QueryResponse> {
...
/**
 * Submits a query. `onActivity` is optional and receives only transport-level
 * signals the transport can actually observe — it is presentation-only and never
 * reports a backend analysis stage as finished.
 */
export async function submitQuery(
  ...
  if (resolveTransport() === "gradio") {
    return submitQueryViaGradio(request, onActivity);
  }
  return submitQueryViaFastapi(request, onActivity);
}
