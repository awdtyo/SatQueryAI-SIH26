...
export default function ChartPanel({ chart }: { chart: ChartEntry[] }) {
        ...
        <ResponsiveContainer width="100%" height="100%">
          {mode === "bar" ? (
            <BarChart data={data}>
              ...
              <YAxis tick={{ fontSize: 10, fill: "#9ca3af" }} domain={[0, 100]} tickFormatter={(v) => `${v}%`} />
              <Tooltip formatter={(v: number) => `${v}%`} contentStyle={{ background: "#1e293b", border: "1px solid #334155", fontSize: 11 }} />
            ...
            <PieChart>
              <Pie data={data} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={(e) => `${e.name} ${e.value}%`}>
                {data.map((_, i) => (
                  <Cell key={i} fill={COLORS[i % COLORS.length]} />
                ))}
              </Pie>
